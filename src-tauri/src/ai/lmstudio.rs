...
impl AIProvider for LMStudioProvider {
    async fn chat(&self, config: &DbAIProvider, request: ChatRequest) -> Result<ChatResponse, String> {
        ...
        println!("[LMStudio Chat] Sending request to URL: {} (Model: {})", url, config.model_name);
        ...
        if !status.is_success() {
            ...
            return Err(format!("LM Studio returned error status: {}. Response: {}", status, err_text));
        }

        let res_json: serde_json::Value = response
            .json()
            .await
            .map_err(|e| format!("Failed to parse LM Studio JSON: {}", e))?;
